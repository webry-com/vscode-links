...
function validateConfig(config: any): config is CustomLinksConfig {
    const result = customLinksConfigSchema.safeParse(config);
    if (!result.success) {
        vscode.window.showErrorMessage(
            `Invalid vscode links configuration. Error: ${result.error}`
        );
    }
...
async function loadConfig(workspacePath: string): Promise<any> {
...
export function convertToFileUrl(filePath: string): string {
    ...
    let urlPath = normalizedPath.replace(/\\/g, "/");
